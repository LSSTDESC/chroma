...
def air_refractive_index(wave, pressure=69.328, temperature=293.15, H2O_pressure=1.067):
    ...
    sigma_squared = 1.0 / (wave * 1.0e-3) ** 2.0  # inverse wavenumber squared in um^-2

    n_minus_one = (
        64.328 + (29498.1 / (146.0 - sigma_squared)) + (255.4 / (41.0 - sigma_squared))
    ) * 1.0e-6
    P = pressure * 7.50061683  # kPa -> mmHg
    T = temperature - 273.15  # K -> C
    W = H2O_pressure * 7.50061683  # kPa -> mmHg
    n_minus_one *= (
        P * (1.0 + (1.049 - 0.0157 * T) * 1.0e-6 * P) / (720.883 * (1.0 + 0.003661 * T))
    )
    n_minus_one -= (
        (0.0624 - 0.000680 * sigma_squared) / (1.0 + 0.003661 * T) * W * 1.0e-6
    )
...
def air_refractive_index2(
    ...
    n_minus_one = (
        64.328
        + (29498.1e-6 / (146e-6 - sigma_squared))
        + (255.4e-6 / (41e-6 - sigma_squared))
    ) * 1.0e-6
    p_ref = 101.325  # kPA
    t_ref = 288.15  # K
