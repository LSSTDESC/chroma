...
class GalTool:
    ...
    def get_image(
        ...
        ring_beta: Union[None, float] = None,
        ring_shear: Union[None, galsim.Shear] = None,
        oversample: int = 1,
    ) -> galsim.Image:
    ...
    def get_PSF_image(self, oversample: int = 1, method: str = "auto") -> galsim.Image:
        ...
        if isinstance(self.PSF, galsim.ChromaticObject):
            star = galsim.Gaussian(fwhm=1.0e-8) * self.SED
    ...
    def get_r2(self, gparam: lmfit.Parameters, oversample: int = 1) -> float:
    ...
    def get_uncvl_r2(self, gparam: lmfit.Parameters, oversample: int = 1) -> float:
    ...
    def compute_AHM(
        self, gparam: lmfit.Parameters, oversample: int = 4
    ) -> Tuple[float, float]:
        ...
        for i in range(10):
            ...
            mx = im.array.max()
            ahms.append(
                self.pixel_scale**2 * (im.array > mx / 2.0).sum() / oversample**2
            )
    ...
    def compute_PSF_AHM(self, oversample: int = 4) -> Tuple[float, float]:
    ...
    def compute_PSF_FWHM(self, oversample: int = 4) -> Tuple[float, float]:
...
class SersicTool(GalTool):
    ...
    def set_FWHM(
        self, gparam: lmfit.Parameters, FWHM: float, oversample: int = 4
    ) -> lmfit.Parameters:
    ...
    def set_r2(
        self, gparam: lmfit.Parameters, r2: float, oversample: int = 4
    ) -> lmfit.Parameters:
    ...
    def get_uncvl_r2(self, gparam: lmfit.Parameters) -> float:
    ...
    def set_uncvl_r2(self, gparam: lmfit.Parameters, r2: float) -> lmfit.Parameters:
    ...
    def get_ring_params(
        self, gparam: lmfit.Parameters, ring_beta: float, ring_shear: galsim.Shear
    ) -> lmfit.Parameters:
        ...
        # complex ellipticity
        c_ellip = gparam["g"].value * complex(
            np.cos(2.0 * rot_phi), np.sin(2.0 * rot_phi)
        )
        ...
        gparam1["x0"].value = gparam["x0"].value * np.cos(ring_beta / 2.0) - gparam[
            "y0"
        ].value * np.sin(ring_beta / 2.0)
        gparam1["y0"].value = gparam["x0"].value * np.sin(ring_beta / 2.0) + gparam[
            "y0"
        ].value * np.cos(ring_beta / 2.0)
        gparam1["g"].value = s_g
        gparam1["phi"].value = s_phi
        return gparam1

    @staticmethod
    def default_galaxy() -> lmfit.Parameters:
    ...
    def use_effective_PSF(self) -> None:
    ...
    def apply_perturbative_correction(
        self, r2byr2=1.0, Vstar=1.0e-8, Vgal=1.0e-8, parang=0.0 * galsim.degrees
    ):
        if isinstance(self.PSF, galsim.ChromaticObject):
            star = galsim.Gaussian(fwhm=1.0e-8) * self.SED
            prof = galsim.Convolve(star, self.PSF)
...
class DoubleSersicTool(GalTool):
    ...
    def get_image(
        self, gparam, ring_beta=None, ring_shear=None, oversample=1, icomp=None
    ):
        """Draw a galaxy image using GalSim.  Potentially rotate and shear the galaxy as part of a
        ring test.  Optionally draw a high-resolution image.
        ...
        if icomp is not None:
            ...
            im1 = self.sersictools[0].get_image(
                self.indiv_gparam(gparam, 0), ring_beta, ring_shear, oversample
            )
            im2 = self.sersictools[1].get_image(
                self.indiv_gparam(gparam, 1), ring_beta, ring_shear, oversample
            )
            return im1 + im2
    ...
    def set_FWHM(self, gparam, FWHM, oversample=4, icomp=None):
        """Set the galaxy PSF-convolved FWHM."""
        if icomp is not None:
            igparam = self.sersictools[icomp].set_FWHM(
                indiv_gparam(gparam, icomp), FWHM, oversample
            )
    ...
    def get_uncvl_r2(self, gparam):
        ...
        @gparam   lmfit.Parameters
        """
        return chroma.component_Sersic_r2(
            [gparam["n_1"].value, gparam["n_2"].value],
    ...
    def get_ring_params(
        ...
        # complex ellipticity
        c_ellip1 = gparam["g_1"].value * complex(
            np.cos(2.0 * rot_phi1), np.sin(2.0 * rot_phi1)
        )
        ...
        gparam1["x0_1"].value = gparam["x0_1"].value * np.cos(ring_beta / 2.0) - gparam[
            "y0_1"
        ].value * np.sin(ring_beta / 2.0)
        gparam1["y0_1"].value = gparam["x0_1"].value * np.sin(ring_beta / 2.0) + gparam[
            "y0_1"
        ].value * np.cos(ring_beta / 2.0)
        ...
        # complex ellipticity
        c_ellip2 = gparam["g_2"].value * complex(
            np.cos(2.0 * rot_phi2), np.sin(2.0 * rot_phi2)
        )
        ...
        gparam1["x0_2"].value = gparam["x0_2"].value * np.cos(ring_beta / 2.0) - gparam[
            "y0_2"
        ].value * np.sin(ring_beta / 2.0)
        gparam1["y0_2"].value = gparam["x0_2"].value * np.sin(ring_beta / 2.0) + gparam[
            "y0_2"
        ].value * np.cos(ring_beta / 2.0)
