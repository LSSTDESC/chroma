...
for i, s in enumerate(spectra):
    ...
    for f in filters:
        ...
        chroma.chroma_fill_between(
            wave, 1.3 * photons_filtered / scale, 0, c=color, axes=ax[i, 0]
        )

        # second axis is photons binned by refraction
        ax[i, 1].plot(R, angle_dens / angle_dens.max() / 1.2, color="black")
        chroma.chroma_fill_between(
            R, angle_dens / angle_dens.max() / 1.2, 0, c=color, axes=ax[i, 1]
        )
        if i == 0:
            ax[1, 1].plot(
                R, angle_dens / angle_dens.max() / 1.2, color="black", alpha=0.2
            )
