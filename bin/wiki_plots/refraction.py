...
# Fourth plot: same as third buy for keynote presentation
...
for zenith in zeniths:
    ...
    refrac_ref = refrac_angle[numpy.argmin(abs(waves - 500))]
    # chroma output is also in radians, so convert to arcsec here
    ax.plot(waves, (refrac_angle - refrac_ref) * 180/numpy.pi * 3600, label=str(zenith)+' deg')
...
fig.savefig('output/DCRfilters_keynote.png', dpi=220)


# Fifth plot: refraction relative to 690 nanometers vs wavelength but only for weak lensing shape
