...
def one_case_study(Sersic_n, bd, gamma, beta,
                   gal_SED_name, star_SED_name, filter_name, redshift):
    size=11
    s_engine = chroma.ImageEngine.GalSimSEngine(size=size,
    ...
    star_PSF = PSF_model(swave, sphotons)
    ...
    #gparam = galtool.set_uncvl_r2(gparam, (0.27/0.5)**2) # 0.27 arcsecond second moment radius

    PSF_FWHM = s_engine.get_PSF_FWHM(star_PSF)
    print 'PSF_FHWM:', PSF_FWHM
    circ_gparam = galtool.circularize(gparam)
    circ_gparam = galtool.set_FWHM(circ_gparam, 1.3 * PSF_FWHM, star_PSF)
    gparam['r_e'].value = circ_gparam['r_e'].value
    print 'gal r_e', gparam['r_e'].value
    ...
    gal_PSF = PSF_model(gwave, gphotons)

    bd_engine = chroma.ImageEngine.GalSimBDEngine(size=size,
                                                  gsp=galsim.GSParams(maximum_fft_size=32768))
    ...
    print 'target |g|: {}'.format(0.2)
    print 'fitted |g|: {}'.format(fitparam['b_gmag'].value)
    # generate fit images
...
if __name__ == '__main__':
    one_case_study(Sersic_n=1.9, bd=True,
                   gamma=0.01+0.02j, beta=(30.0*np.pi/180),
                   gal_SED_name='CWW_E_ext', star_SED_name='uko5v', filter_name='Euclid_350',
                   redshift=1.7)
