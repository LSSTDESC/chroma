...
def image_comparison(args):
    ...
    bandpass = bandpass.thin(1.0e-4)

    gal_SED = (
        galsim.SED(args.datadir + args.spec)
        .atRedshift(args.redshift)
        .withFlux(1.0, bandpass)
    )
    ...
    PSF = galsim.ChromaticAtmosphere(
        PSF685,
        base_wavelength=685.0,
        zenith_angle=args.zenith_angle * galsim.degrees,
        alpha=0.0,
    )
    gtool = chroma.ChromaticSersicTool(
        gal_SED, bandpass, PSF, args.stamp_size, args.pixel_scale
    )
    ...
    # fast method
    fast_gtool = chroma.FastChromaticSersicTool(
        gal_SED, bandpass, PSF, args.stamp_size, args.pixel_scale
    )
    fast_image = fast_gtool.get_image(
        gparam, ring_shear=galsim.Shear(g1=args.g1, g2=args.g2)
    )
    ...
    print(
        "      {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s} {:>12s}".format(
    ...
    if args.plot:
        ...
        im4 = chroma.my_imshow(
            np.log10(abs(image.array - fast_image.array) / image.array), ax
        )
        plt.colorbar(im4)
        plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datadir", default="../data/", help="directory to find SED and filter files."
    )
    parser.add_argument(
        "--PSF_beta",
        type=float,
    ...
    parser.add_argument(
        "-n", "--sersic_n", type=float, default=0.5, help="Sersic index (Default 0.5)"
    )
    parser.add_argument(
        "--gal_ellip",
        type=float,
    ...
    parser.add_argument(
        "--g1", type=float, default=0.0, help="Apply '+' shear g1 to galaxy"
    )
    parser.add_argument(
        "--g2", type=float, default=0.0, help="Apply 'x' shear g2 to galaxy"
    )
